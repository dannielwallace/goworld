...
func (bot *ClientBot) loop() {
	...
	for {
		...
		case <-ticker:
			...
			if true {
				now := time.Now()
				if now.Sub(bot.syncPosTime) > time.Millisecond * 1000 {
					bot.sendSyncPosition2Gs()
					bot.syncPosTime = now
				}
			}
...
func (bot *ClientBot) sendSyncPosition2Gs() {
	pkt := netutil.NewPacket()
	pkt.AppendUint16(proto.MT_REDIRECT_TO_GS_START)
	pkt.AppendUint32(1)
	pkt.AppendUint32(2)
	pkt.AppendUint32(3)
	pkt.AppendUint32(4)

	bot.conn.SendPacketRelease(pkt)
}
